...
CREATE TABLE IF NOT EXISTS receipts (
  ...
  CONSTRAINT fk_receipts_tx FOREIGN KEY (transaction_id) REFERENCES transactions(id) ON DELETE CASCADE
) ENGINE=InnoDB ROW_FORMAT=DYNAMIC;
...
VALUES ('ADMIN', 'Administrador'), ('INGRESADOR', 'Ingresador'), ('VISUALIZADOR', 'Visualizador')
...
SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN ('transactions.read','transactions.write','reports.read','dashboard.read','admin.full') WHERE r.code = 'ADMIN';

INSERT IGNORE INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN ('transactions.read','transactions.write','reports.read','dashboard.read') WHERE r.code = 'INGRESADOR';

INSERT IGNORE INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN ('transactions.read','reports.read','dashboard.read') WHERE r.code = 'VISUALIZADOR';
